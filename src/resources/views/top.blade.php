...
<html lang="{{ str_replace("_", "-", app()->getLocale()) }}">
  ...
  <body class="font-sans antialiased">
    <div class="min-h-dvh bg-gray-100 dark:bg-gray-900" x-data="{ loginDrawer: {{ $errors->any() ? "true" : "false" }} }">
      <header class="bg-white shadow dark:bg-gray-800">
        <div class="relative mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <div class="flex justify-center sm:justify-start">
            <h2 class="text-2xl font-semibold leading-tight text-gray-800 dark:text-gray-200">狩野健一のWebサイト</h2>
          </div>
      ...
      <!-- Drawer Content -->
      <div
        class="fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-[10px] border bg-white shadow-lg dark:bg-gray-800"
        style="display: none"
        x-show="loginDrawer"
      ...
      >
        ...
        <div class="mx-auto w-96 max-w-full p-6">
          <form method="POST" action="{{ route("login") }}">
            @csrf
            <div class="mb-4">
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300" for="email">メールアドレス</label>
              <input
                ...
                required
                autofocus
                x-init="$watch('loginDrawer', (value) => value && setTimeout(() => $el.focus(), 100))"
              />
              @error("email")
            ...
            <div class="mb-4">
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300" for="password">パスワード</label>
              <input
                class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                id="password"
                name="password"
                ...
                <ul class="space-y-4">
                  <li class="rounded-lg bg-white shadow-md transition-shadow duration-300 hover:shadow-lg dark:bg-gray-800">
                    <a class="block rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700" href="/gen">
                      <div class="text-lg font-semibold text-yellow-600 dark:text-yellow-400">幻想郷Radioリアルタイム</div>
                  ...
                  <li class="rounded-lg bg-white shadow-md transition-shadow duration-300 hover:shadow-lg dark:bg-gray-800">
                    <a class="block rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700" href="/thssren.html">
                      <div class="text-lg font-semibold text-yellow-600 dark:text-yellow-400">thssren(東方スクショリネーマー)</div>
                      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">歯抜けになった東方スクショファイルを000から振りなおします。</p>
                    </a>
                  </li>
                  <li class="rounded-lg bg-white shadow-md transition-shadow duration-300 hover:shadow-lg dark:bg-gray-800">
                  ...
                  <li class="rounded-lg bg-white shadow-md transition-shadow duration-300 hover:shadow-lg dark:bg-gray-800">
                    <a class="block rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700" href="/a5m2reverse.html">
                      <div class="text-lg font-semibold text-yellow-600 dark:text-yellow-400">ReverseDDL</div>
                      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">A5:SQL Mk-2のテーブル定義書からCREATE TABLE文を逆生成します。</p>
                    </a>
                  </li>
                </ul>
              </div>
                ...
                <div class="pl-4 text-sm">
                  @foreach ($articles as $article)
                    ...
                    >
                      ...
                      <a
                        class="text-gray-700 transition-colors group-hover:text-yellow-600 dark:text-gray-300 dark:group-hover:text-yellow-400"
